...
function createLineBuffer(maxBytes) {
  return {
    ...
    append(chunk) {
      ...
      for (let i = 0; i < chunk.length; i++) {
        ...
        switch (this.escState) {
          ...
          case 3: // OSC/string: ends at BEL (0x07) or ESC \\ (0x1b 0x5c)
            ...
            break;
          case 5:
            // Inside a string (OSC/DCS/…), just saw ESC. ESC \\ terminates it; otherwise the ESC
            // begins a fresh sequence (string was malformed) — re-dispatch instead of swallowing.
      ...
      // Bound pending too: a non-alt-screen program can emit megabytes without a newline (e.g. a
      // \r-only progress stream). Flush it as a pseudo-line past PENDING_FLUSH_BYTES so totalBytes
      // accounting catches it and the ring buffer can trim. Replay stays verbatim; only the "line"
      // boundary is synthetic. (During alt-screen we don't accumulate pending, so this won't fire.)
      const PENDING_FLUSH_BYTES = 256 * 1024;
      if (this.pending.length > PENDING_FLUSH_BYTES) {
        this.lines.push({ data: this.pending, bytes: this.pending.length });
